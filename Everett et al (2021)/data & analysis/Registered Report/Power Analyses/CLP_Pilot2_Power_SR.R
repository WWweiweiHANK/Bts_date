...
print(round(summary(trust_model)$coeff, 2))

se = summary(trust_model)$coeff["argument_recode:dimension_recode", "Std. Error"]
sd = se*sqrt(n)
print(paste("SEM:", round(sd, 2)))

## Extend to predefined subjects
trust_model_ext <- extend(trust_model, n=nsubs, along='subId')

## Power Analysis for a variety of effect sizes
summ <- data.frame()
for (cd in seq(0.02, 0.07, .01)) {
  
  print(paste("Cohen's d:", cd))
  slope <- cd*sd
  print(paste("Slope:", round(slope, 2)))
  fixef(trust_model_ext)['argument_recode:dimension_recode'] <- slope
  
  power <- powerSim(trust_model_ext, test=fixed('argument_recode:dimension_recode', 'z'),
                    nsim = nsim, alpha=alpha, progress=show_progress)
  
  print(power)
  summ <- rbind(summ, cbind(task='selfrep', nsubs=nsubs, 
                            d=round(cd, 2), slope=round(slope, 2), 
                            summary(power)))
  
  # For tracking progress
  write.csv(summ, "CLP_Pilot2_Power_SR_tmp.csv", row.names = FALSE)
  
}

write.csv(summ, "CLP_Pilot2_Power_SR.csv", row.names = FALSE)
