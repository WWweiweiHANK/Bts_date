...
mlm_test <- function(simNum, 
  ...
  random1 <- ~ 1 
  random_param1 <- list(random_var = c(pi^2/pi_2), 
                        rand_gen = 'rnorm', 
                        ther_sim = TRUE)
  ...
  dat22<-simglm::sim_glm(fixed = fixed1, 
  ...
  return <- tryCatch({
    ...
    mod2 <-   lme4::glmer(sim_data ~ 1 + child + gender + education + employment + income + age +
                            GDP + GINI +(1|clustID), 
                          data=dat22, 
                          family="binomial")
    
    p2 <- coef(summary(mod2))[2,4]
    p3 <- coef(summary(mod2))[3,4]
    p4 <- coef(summary(mod2))[4,4]
    p5 <- coef(summary(mod2))[5,4]
    p6 <- coef(summary(mod2))[6,4]
    p7 <- coef(summary(mod2))[7,4]
    p8 <- coef(summary(mod2))[8,4]
